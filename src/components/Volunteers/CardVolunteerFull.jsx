...
import {notify_error, notify_success, _fetch, PlacesAutocomplete} from '../../common.js'
...
export const CardVolunteerFull = (props) => (
  <div>
    ...
    <Button onClick={() => props.refer._updateW9Ambassador(props.volunteer, true)}>Has W9</Button>
    <Button onClick={() => props.refer._updateW9Ambassador(props.volunteer, false)}>
      Does Not Have W9
    </Button>
    <div>Currently Approved: {props.volunteer.approved ? 'Yes' : 'No'}</div>
    <div>
      <Button onClick={() => props.refer._approveAmbassador(props.volunteer, false)}>Deny</Button>
      <Button onClick={() => props.refer._approveAmbassador(props.volunteer, true)}>Approve</Button>
    </div>
    Currently Admin: {props.volunteer.admin ? 'Yes' : 'No'}
    <div>
      <Button onClick={() => props.refer._adminizeAmbassador(props.volunteer)}>Make Admin</Button>
    </div>
    ...
    {props.volunteer.quiz_results ? (
      props.volunteer.quiz_results.map ? (
        ...
        Object.keys(props.volunteer.quiz_results).map((key) => {
          return (
            <div key={key}>
              <b>question:</b> {key} <b>answer:</b> {props.volunteer.quiz_results[key]}
              <br />
              <br />
            </div>
          )
    ...
    <div style={{maxWidth: '100%'}}>
      <MaterialTable
        columns={[
          ...
          {title: 'Social Connection', field: 'social'},
        ]}
        data={[
          {
            full_name: props.volunteer.full_name,
            email: props.volunteer.email,
            phone: props.volunteer.phone,
            social: '',
          },
        ]}
        title="Claimed Triplers (Ambassador is here as Placeholder)"
      />
    </div>
  </div>
)
